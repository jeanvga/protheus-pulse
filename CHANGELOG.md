...
- **HTTPS no painel.** A tela de rede oferecia abrir o painel para outros computadores e recomendava HTTPS — que o produto não tinha como configurar. Quem ligava a opção passava a trafegar senha e token de sessão em texto claro na rede do cliente, seguindo um conselho que o próprio produto não deixava cumprir. Agora dá para apontar um `.pfx` com a chave privada ou gerar um certificado para a própria máquina em um clique. O certificado é conferido ao salvar, e a gravação é recusada se ele não servir, em vez de deixar o serviço subir sem conseguir atender ninguém; se mesmo assim ele falhar na inicialização, o serviço cai para HTTP e registra o motivo no rastro de partida, em vez de ficar inacessível sem caminho de volta pela tela. A senha do certificado fica cifrada com Data Protection, fora do `network.json`.
- **Backup** em Configurações: um pacote com o banco, a chave de Data Protection e a configuração de rede, gerado com um clique e baixável pelo painel. Não havia caminho nenhum no produto para isso — e a chave é o item que costuma faltar num backup feito à mão: sem ela, o banco restaurado perde as URLs dos pontos de contato e a senha do SMTP, que ficam cifradas por ela. O banco sai por `VACUUM INTO`, consistente mesmo com a coleta rodando; os dez pacotes mais recentes ficam guardados. A restauração continua manual de propósito, com o passo a passo dentro do pacote: trocar o banco embaixo de um serviço em execução corrompe o que estiver sendo escrito.
- Limites do servidor editáveis em Configurações. Os seis valores que classificam processador, memória e disco só mudavam editando o `appsettings.json` e reiniciando o serviço. Ao salvar, as opções que os coletores já receberam são atualizadas no lugar, então o ajuste vale no ciclo seguinte.
- Aba **Jobs** gerenciando heartbeats: cadastro, rotação de token e remoção. As quatro rotas existiam, eram testadas e não tinham interface — cadastrar um job monitorado só era possível por `curl`. A tolerância exibida era fixa em "5 min" no código, independentemente da cadastrada.
